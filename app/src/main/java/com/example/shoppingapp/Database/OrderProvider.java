...
public class OrderProvider extends ContentProvider {
    ...
    private Uri insertCart(Uri uri, ContentValues values) {
        ...
        String name = values.getAsString(OrderContract.OrderEntry.NAME);
        if(name == null) {
            throw new IllegalArgumentException("Name is Required");
        }

        String quantity = values.getAsString(OrderContract.OrderEntry.QUANTITY);
        if(quantity == null) {
            throw new IllegalArgumentException("quantity is Required");
        }

        String price = values.getAsString(OrderContract.OrderEntry.PRICE);
        if(price == null) {
